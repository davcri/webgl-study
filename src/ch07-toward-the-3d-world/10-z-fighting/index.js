...
class App extends WebGLApp {
  ...
  static initVertices() {
    const triangle1 = [
      // x y z , r g b
      0.0, 2.5, -5, 0.0, 1.0, 1.0,
      -2.5, -2.5, -5, 0.0, 1.0, 0.0,
      2.5, -2.5, -5, 0.0, 1.0, 0.0,
    ];

    const triangle2 = [
      0.0, 3.0, -5, 1.0, 0.0, 0.0,
      -3.0, -3.0, -5, 1.0, 1.0, 0.0,
      3.0, -3.0, -5, 1.0, 1.0, 0.0,
    ];
  ...
  draw() {
    ...
    this.gl.drawArrays(this.gl.TRIANGLES, 0, this.n / 2);
    this.gl.polygonOffset(1.0, 0.0);
    this.gl.drawArrays(this.gl.TRIANGLES, this.n / 2, this.n / 2);
  }

  addNotes() {
    const el = document.getElementsByClassName('notes').item(0);
    el.innerHTML = `WARNING: I'm not sure this example
    is working as intended.<br>
    If you rotate the triangles with keys, z-fighting is still occurring.`;
  }
}
