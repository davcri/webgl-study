...
class App extends WebGLApp {
  ready() {
    ...
    const u_ModelViewMatrix = this.u_ModelViewMatrix = this.getUniformLocation('u_ModelViewMatrix');
  ...
  process() {
    // update model view matrix rotation
    const period = 1000;
    this.modelMatrix.setRotate(-2 * Math.sin(this.time / period), 0, 0, 1);
    this.modelViewMatrix = this.viewMatrix.multiply(this.modelMatrix);
